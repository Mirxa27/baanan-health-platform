...
export function I18nProvider({ children, initialLocale }: I18nProviderProps) {
  ...
  const t = (key: string, namespace: Namespace = "common"): string => {
    ...
    return (
      namespaceTranslations?.[key as keyof typeof namespaceTranslations] || key
    );
  };
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const cookieMatch = document.cookie.match(/(?:^|; )locale=(en|ar)/);
      const cookieLocale = (cookieMatch ? cookieMatch[1] : undefined) as
        | Locale
        | undefined;
      ...
      const finalLocale =
        savedLocale && (savedLocale === "en" || savedLocale === "ar")
          ? savedLocale
          : cookieLocale && (cookieLocale === "en" || cookieLocale === "ar")
            ? cookieLocale
            : browserLocale === "ar"
              ? "ar"
              : "en";
